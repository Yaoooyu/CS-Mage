...
sample-level evaluation protocols. It records the assignment of every
`sample_id` to each MSA fold and to the ASR split, allowing users to reproduce
the reported partitions without relying on local paths or data-file order.
