...
- `assets/`: tokenizer configuration and vocabulary.
- `configs/`: MAGE-Fusion, ablation, and ASR records.
- `splits/`: fixed sanitized MSA five-fold and ASR train/validation/test split
  metadata, including `id_split_index.csv` for ID-based lookup.

- `docs/DATA_ACCESS_AND_ETHICS.md`: dataset download locations, permitted use,
