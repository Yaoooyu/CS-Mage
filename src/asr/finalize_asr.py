...
def main() -> None:
    ...
    report=["# Final CS-Mage Changsha Dialect ASR Report", "", "## Protocol", "- Fixed sample-level duration-stratified split, seed=42: train/val/test = 760/160/165.", "- All listed results use the same test manifest, `normalize_text.py`, and corpus-level CER (total character substitutions + deletions + insertions divided by total reference characters).", "- No test-set decoding-parameter search was performed. FunASR baselines used `language=zh`; SenseVoice metadata tags were removed before scoring.", "", "## Main results", "", "| Model | Setting | CER | WER | SER | RTF |", "|---|---|---:|---:|---:|---:|"]
