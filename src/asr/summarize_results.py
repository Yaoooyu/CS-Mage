...
def main() -> None:
    ...
    report = ["# CS-Mage Changsha Dialect ASR Benchmark", "", "## Data validation", f"- {validation['n_samples']} WAV files were readable and matched to the manifest records.", f"- Observed WAV format: {validation['formats']}.", f"- Fixed duration-stratified split (seed 42): train/val/test = {dict(validation['split_counts'])}.", "- `raw_text` is non-empty for all samples and contains Chinese utterance text. Direct ASR checks show exact and near-exact examples, but its status remains candidate reference transcription pending manual audit.", "", "## Direct-inference results", "", "| Model | CER | WER | SER | RTF |", "|---|---:|---:|---:|---:|"]
